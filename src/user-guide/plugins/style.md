...
第一种，右键任意一个列的名字，会弹出一个右键菜单，并且在最后有两个按钮，一个是`列设置`，一个是`视图组`。
...
如果你没有看到`Secret Key`这一项，那么你也可以点击`编辑` -> `首选项` / `设置` -> `高级`-> `编辑器`，在弹出的窗口中输入`easyscholar`，并搜索，双击搜索结果后，填写 easyScholar 密钥，并点击 OK。
