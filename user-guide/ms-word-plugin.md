...
![Word 文档首选项窗格](../assets/images/word文档首选项窗格.png)

「文档首选项」窗口允许您设置以下针对该文档的设置：

1. **「参考文献样式」**：一般情况下，你只需要修改这一项，其余设置均保持默认即可。
...
打开该对话框后，对话框会默认展示出你在 Zotero 文库列表中已经选中的条目和正在阅读的条目。如果这些条目正是你想要选择的，可以直接单击以选择。

![未搜索关键词的引文对话框](../assets/images/word-引文对话框.png)

你也可以在搜索框中键入标题的一部分、一个或多个作者的姓氏和/或年份。匹配项将立即显示在对话框下方。

![键入关键词的引文对话框](../assets/images/word-引文对话框2.png)

单击某条目可以选择这个条目，也可以用键盘「↑↓」+「Enter」键来选择条目。选中的条目以气泡形状显示在对话框中。

![添加了引注的引文对话框](../assets/images/word-引文对话框3.png)
...
完成选择后，再次按「Enter/Return」键以插入引注并关闭引注对话框。

### 文库模式

「添加引注」对话框有「列表模式」和「文库模式」两种。前者主要通过关键词检索文献，后者可以通过合集来查找文献。

你可以在对话框右下角切换两种模式，Zotero 默认使用上次使用过的模式，但也可以在设置中指定默认模式。

![引文对话框 - 文库模式](../assets/images/word-引文对话框-文库模式.png)

### 引注顺序

某些引文样式要求一个位置内文献按字母顺序排序（例如，`(Doe 2000，Grey 1994，Smith 2008)`）或按时间顺序排序（`(Grey 1994，Doe 2000，Smith 2008)`）。Zotero 将自动遵循这些排序规则。

::: details 禁用文献自动排序？

若要禁用引文中引文的自动排序，请拖动引文以在「添加引文」框中重新排列引文。您也可以单击「添加引文」框右下角的「引用设置」图标，然后取消选中「保持文献自动排序」选项。_此选项仅对指定引文排序顺序的引文样式显示。_ 要恢复自动排序，请重新选中「保持源已排序」选项。

![引文对话框禁用自动排序](../assets/images/word-引文对话框-禁用自动排序.png)

:::

### 孤立的条目

孤立的条目是指存在于 Word 文档中但不存在于 Zotero 文库的参考文献条目。
...
在「添加引注」对话框中，您还可以单击被引用条目的气泡，然后编辑该引用条目的更多信息，如页码等定位符、前后缀、省略作者等。请参考 [自定义引注](#自定义引注)。

![编辑引注](../assets/images/word-引文对话框-编辑引注.png)
