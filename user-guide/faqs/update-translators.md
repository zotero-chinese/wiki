...
如果您将 Zotero 的 `数据存储位置/Data Directory` 自定义为网盘的同步文件夹中，可能在某些情况下出现转换器文件被同步盘锁定，无法更新的情况。考虑到直接同步 Zotero 数据文件夹的各种风险，请您停止同步，并根据 [自定义数据文件夹](../faqs/custom-data-directory) 的教程将数据文件夹迁移到本地的其他位置，且确保未使用任何工具直接同步这一文件夹。
