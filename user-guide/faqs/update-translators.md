...
2. 进入茉莉花插件的设置，转到 「中文转换器设置」 部分，然后点击 「立即更新转换」 按钮。
   ...
   Zotero 7 的正式版已经发布，由于 Zotero 6 的插件已经基本停止维护，可能会遇到兼容性问题，建议您在做好[备份](../backup.md)后，尽快[升级到 Zotero 7](https://zotero-chinese.com/blog/posts/hello-zotero-7)。
...
4. 中文文献文件名不规范：中文文献的元数据检索依赖文件名，文件名不规范可能导致检索失败。建议将文件名改为文献标题然后再重新尝试检索元数据。
5. 不支持抓取或其他问题：不管是中文文献还是英文文献，都有可能会遇到检索失败的情况（特别是对于图书专著）。如果检索失败，建议采用其他方式获得文献条目，然后将 PDF 文件以附件的形式添加到该条目上（将附件拖到该条目上即可，但不要拖到条目之间的间隙里）。
