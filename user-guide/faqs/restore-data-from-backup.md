...
在对数据文件夹进行任何操作之前，都建议您先对数据文件夹进行备份，以备不时之需。备份教程详见：[Zotero 备份教程](../backup)。
...
如果您之前启用了 Zotero 同步功能，且本地库为空（或新建了 Zotero 数据文件夹），可以通过同步来恢复您的在线库数据。

1. **验证在线库数据完整性**  
   首先访问 [Zotero 官网](https://www.zotero.org/user/login) 登录您的账户，确认在线库中的数据是否正确。

2. **设置同步账户**  
   打开 Zotero，点击「编辑」→「设置」→「同步」，输入您的用户名和密码。
...
如果您有 Zotero 数据目录的手动备份，可以用它替换当前的数据目录来恢复您的库。

1. **查看当前数据目录位置**  
   打开 Zotero 的「编辑」→「设置」→「高级」→「文件和文件夹」，记下「数据存储位置」的路径（默认是您的用户文件夹中的 `Zotero` 目录）。
2. **打开数据目录**  
   点击「打开数据文件夹」按钮，这将打开包含 `zotero.sqlite` 文件和可能存在的 `storage` 子目录的当前数据目录。

3. **备份当前数据目录**  
   关闭 Zotero，进入数据目录的上级文件夹，将当前数据目录重命名为 `Zotero-Old`。

4. **替换为备份数据**  
   将您的备份数据目录复制到原来的位置（例如，复制到重命名前的 `Zotero` 位置）。

5. **重新启动 Zotero**  
   重新打开 Zotero，您应该能看到恢复的数据。

::: tip

恢复成功后，您可以删除 `Zotero-Old` 文件夹，但建议先保留一段时间，直到确定恢复的数据无误。

:::

::: warning

如果您使用同步功能，恢复后再次同步时，备份后到恢复前的这段时间内所做的任何更改（如果已同步到在线库）将被应用到恢复的数据库中。如果不希望这种情况发生，请参考下一节。
...
5. **重新启用同步**  
   确认恢复成功后，重新启用自动同步。保留 `zotero.sqlite.old` 和您的 `.bak` 文件备份，直到确认所有数据都完好无损并在所有计算机上同步。
